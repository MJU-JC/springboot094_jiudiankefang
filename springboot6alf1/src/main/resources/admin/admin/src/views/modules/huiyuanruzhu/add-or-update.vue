<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="客房号" prop="kefanghao">
          <el-select  @change="kefanghaoChange" v-model="ruleForm.kefanghao" placeholder="请选择客房号">
            <el-option
                v-for="(item,index) in kefanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.kefanghao" label="客房号" prop="kefanghao">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="入住押金" prop="ruzhuyajin">
          <el-input v-model="ruleForm.ruzhuyajin" 
              placeholder="入住押金" clearable  :readonly="ro.ruzhuyajin"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="入住押金" prop="ruzhuyajin">
              <el-input v-model="ruleForm.ruzhuyajin" 
                placeholder="入住押金" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="支付方式" prop="zhifufangshi">
          <el-input v-model="ruleForm.zhifufangshi" 
              placeholder="支付方式" clearable  :readonly="ro.zhifufangshi"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kefangzhuangtai = true;
          ...
          if(o=='ruzhuyajin'){
            this.ruleForm.ruzhuyajin = obj[o];
	    this.ro.ruzhuyajin = true;
            continue;
          }
          if(o=='zhifufangshi'){
    ...
    kefanghaoChange () {
      this.$http({
        url: `follow/kefangxinxi/kefanghao?columnValue=`+ this.ruleForm.kefanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.kefangleixing){
                this.ruleForm.kefangleixing = data.data.kefangleixing
              }
              if(data.data.suoshujiudian){
                this.ruleForm.suoshujiudian = data.data.suoshujiudian
              }
    ...
    zhanghaoChange () {
      this.$http({
        url: `follow/huiyuan/zhanghao?columnValue=`+ this.ruleForm.zhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.kefangleixing){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huiyuanruzhu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuanruzhuCrossAddOrUpdateFlag = false;
                  this.parent.search();
